...
<body class="text-gray-800 min-h-screen flex items-center justify-center p-4 selection:bg-gray-200">
        ...
        <li class="task-item group flex items-start justify-between p-5 hover:bg-gray-50 transition-colors duration-200">
          <div class="flex items-start gap-4 flex-1 min-w-0">
            ...
            <form action="{{ route('tasks.done', $task->id) }}" method="POST">
              @csrf
               <button type="submit" class="mt-1 transition-colors text-xl {{ $task->done ? 'text-[#1a1a1a]' : 'text-gray-300 hover:text-[#1a1a1a]' }}" aria-label="{{ $task->done ? '未完了に戻す' : '完了にする' }}">
                <i class="fa-regular {{ $task->done ? 'fa-square-check' : 'fa-square' }}"></i>
              </button>
            </form>
          ...
          <form action="{{ route('tasks.destroy', $task->id) }}" method="POST"> @method('DELETE')
            @csrf
  ...
  <a href="{{ route('tasks.create') }}" class="fixed bottom-8 right-8 w-14 h-14 bg-[#1a1a1a] text-white rounded-full shadow-lg hover:shadow-xl hover:bg-black hover:scale-105 transition-all duration-300 flex items-center justify-center group z-50 focus:outline-none focus:ring-4 focus:ring-gray-200">
